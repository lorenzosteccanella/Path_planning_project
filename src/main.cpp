...
// have a reference target velocity  
double ref_vel = 0.0;
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		int min_s_line_0 = INT_MAX;
		int min_s_line_1 = INT_MAX;
		int min_s_line_2 = INT_MAX;
		...
		// previous path point added to the planner
		...
		double x_add_on=0;
		  ...
		  // rotate back to normal from car coordinate to world coordinate
